...
static int uart_sam0_init(struct device *dev)
{
	...
	struct device *gclk = device_get_binding(cfg->gclk_src);
	u32_t gclk_freq;

	clock_control_on(gclk, SOC_ATMEL_SAM0_GCLK_SUBSYS(cfg->gclk_id));
	...
	clock_control_get_rate(gclk, SOC_ATMEL_SAM0_GCLK_SUBSYS(cfg->gclk_id),
			       &gclk_freq);

	retval = uart_sam0_set_baudrate(usart, cfg->baudrate, gclk_freq);
	if (retval != 0) {
		return retval;
	}
...
static const struct uart_sam0_dev_cfg uart_sam0_config_##n = {		       \
	...
	.gclk_src = DT_UART_SAM0_SERCOM##n##_CLOCK_CONTROLLER,		       \
