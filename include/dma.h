 ...
 * pending_length : how many data are pending to be transferred.
 *
 */
struct dma_status {
	bool busy;
 ...
 * @retval Negative errno code if failure.
 */
__syscall int dma_get_status(struct device *dev, u32_t channel,
			     struct dma_status *stat);

static inline int z_impl_dma_get_status(struct device *dev, u32_t channel,
					struct dma_status *stat)
{
	const struct dma_driver_api *api =
		(const struct dma_driver_api *)dev->driver_api;
