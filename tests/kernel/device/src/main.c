...
void test_linked_device(void)
{
	...
	linked = device_get_linked(NULL, DEVICE_LINK_CLOCK, 0);
	zassert_equal(linked, NULL, NULL);

	dev = device_get_binding(DUMMY_PORT_2);
	zassert_false((dev == NULL), NULL);

	linked = device_get_linked(dev, DEVICE_LINK_DMA, 0);
	zassert_equal(linked, NULL, NULL);
	linked = device_get_linked(dev, DEVICE_LINK_CLOCK, 1);
	zassert_equal(linked, NULL, NULL);
	linked = device_get_linked(dev, DEVICE_LINK_CLOCK, 0);
